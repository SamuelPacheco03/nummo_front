...
/*
  El test que justifica que `azul` exista como candidata: si las 18 ranuras reproducen
...
test('las tres candidatas rellenan las 19 ranuras en los dos modos', () => {
  for (const palette of PALETTES) {
    for (const mode of ['light', 'dark'] as const) {
      ...
      expect(Object.keys(slots), `${palette.id}/${mode}`).toHaveLength(19)
    }
  }
})
