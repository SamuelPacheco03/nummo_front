...
- **Handoffs con endpoints/flujos/tokens:** `./contract/HANDOFF-fase-0.md` … `HANDOFF-fase-10.md`. **Léelos por área.**
...
El backend ya cerró **todas sus fases (V1)**: `openapi.json` aquí incluido está en **v1.0.0 con 127 paths / 160 operaciones** (auth, orgs, contactos/maestros, cartera, pagos, mora, gastos/egresos, caja/transferencias, reportes). Corre `pnpm api:gen` para regenerar el cliente. Detalle en **`./contract/SYNC-STATUS.md`**. Si el backend hace algún ajuste puntual, se recopiará `openapi.json` y se actualizará ese archivo.
