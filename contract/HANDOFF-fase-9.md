...
El modelo objetivo tiene cuatro capas. **La Fase 1 (permiso) está mergeada a `dev`; la Fase 2 está construida en `feat/platform-entitlements`.**
...
## Lo que se hizo en la Fase 2 (rama `feat/platform-entitlements`)
...
## Fase 2 — Entitlements y planes · rama `feat/platform-entitlements` · ✅ hecha
...
- Los cuatro planes entran por **seed** (`scripts/seed-plans.ts`), no por migración — misma convención que `scripts/seed.ts`. Editar un plan después es un UPDATE desde el panel de superadmin, no un deploy.
...
## Fase 3 — Límites de flujo · rama `feat/platform-usage` · ✅ hecha
...
## Fase 4 — Superadmin · rama `feat/platform-admin` · ✅ hecha
...
## Fase 5 — Roles personalizados · `feat/custom-roles`

`roles` + `role_permissions` por organización, gated por el entitlement `custom_roles`. El `PermissionResolver` lee de DB con caché invalidada por `updatedAt`. Validación al crear rol: el set pedido ⊆ permisos asignables, con `OWNER_ONLY_PERMISSIONS` fuera por construcción. Es aquí donde se añaden guards `.read` a los GET que lo justifiquen (ver utilidad, ver cuentas bancarias).

## Fase 6 — Aprobaciones por umbral · `feat/expense-approvals`

"Todo egreso mayor a $5.000.000 requiere aprobación" **no lo resuelve un permiso**: necesita estado en la entidad (`PENDING_APPROVAL → APPROVED/REJECTED`), umbral configurable en `Money`, y un desembolso que no pueda ejecutarse sin aprobación previa. `expenses.approve` es condición necesaria, no suficiente. Fase de producto, no de seguridad.

## Estado del proyecto

Backend V1 + verticalización + BYOK + Numi (Fases A–D) + Fases 1 a 3 en `dev` + Fase 4 (consola de plataforma, en `feat/platform-admin`). **375 tests verdes** (58 archivos), `openapi.json` sincronizado y anotado con `x-required-permission` y `x-required-feature`. `main` sigue en V1 y no se mergea hasta tener todo funcional; el trabajo va por `dev` con una rama por fase.

**Al desplegar la Fase 2, en este orden**: `pnpm db:migrate` → `pnpm seed:plans` (siembra + backfill) → arriba la app. Si el seed no llegara a correr, `snapshot()` cae a cero features y cero topes: nadie pierde acceso a sus datos, simplemente no hay features de pago hasta que existan las filas.
